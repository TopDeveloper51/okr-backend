...
public class DutyController {
	...
	@Autowired
	private DutyMapper mService;

	@RequestMapping(value = "/getByFields", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<?> selectByFields(@RequestBody Duty param) {
		try {
			List<Duty> ret = mService.selectByFields(param);
	...
	public ResponseEntity<?> addOne(@RequestBody Duty param) {
		try {
			...
			if (ret == 0) {
				...
				Duty newObj = new Duty();
				newObj.duty_id = param.duty_id;
				List<Duty> retObj = mService.selectByFields(newObj);
	...
	public ResponseEntity<?> deleteByFields(@RequestBody Duty param) {
		try {
	...
	public ResponseEntity<?> updateByFields(@RequestBody Duty param) {
		try {
