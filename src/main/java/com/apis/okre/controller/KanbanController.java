...
public class KanbanController {
	...
	@Autowired
	private KanbanService mService;

	@RequestMapping(value = "/getByFields", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<?> selectByFields(@RequestBody Kanban param) {
		try {
			List<Kanban> ret = mService.selectByFields(param);
	...
	public ResponseEntity<?> addOne(@RequestBody Kanban param) {
		try {
			...
			if (ret == 0) {
				...
				Kanban newObj = new Kanban();
				newObj.kb_id = param.kb_id;
				List<Kanban> retObj = mService.selectByFields(newObj);
	...
	public ResponseEntity<?> deleteByFields(@RequestBody Kanban param) {
		try {
	...
	public ResponseEntity<?> updateByFields(@RequestBody Kanban param) {
		try {
