...
public class TeamController {
	...
	@Autowired
	private TeamService mService;

	@RequestMapping(value = "/getByFields", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<?> selectByFields(@RequestBody Team param) {
		try {
			List<Team> ret = mService.selectByFields(param);
	...
	public ResponseEntity<?> addOne(@RequestBody Team param) {
		try {
			...
			if (ret == 0) {
				...
				newObj.team_id = param.team_id;
				List<Team> retObj = mService.selectByFields(newObj);
	...
	public ResponseEntity<?> deleteByFields(@RequestBody Team param) {
		try {
	...
	public ResponseEntity<?> updateByFields(@RequestBody Team param) {
		try {
