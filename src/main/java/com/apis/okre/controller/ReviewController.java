...
import com.apis.okre.mapper.ReviewProblemMapper;
...
public class ReviewController {
	...
	public ResponseEntity<?> selectByFields(@RequestBody Review param) {
		try {
			List<Review> ret = mService.selectByFields(param);
	...
	public ResponseEntity<?> addOne(@RequestBody Review param) {
		try {
			...
			if (ret == 0) {
				...
				newObj.re_id = param.re_id;
				List<Review> retObj = mService.selectByFields(newObj);
	...
	public ResponseEntity<?> deleteByFields(@RequestBody Review param) {
		try {
	...
	public ResponseEntity<?> updateByFields(@RequestBody Review param) {
		try {
			...
			if (ret == 0) {
				...
				List<Review> retObj = mService.selectByFields(newObj);
				return new ResponseEntity<String>(
						Utils.getResponseObject(Constants.UPDATE_SUCCESSFULLY, retObj.get(0)).toString(),
						HttpStatus.OK);
			}
		} catch (Exception e) {
			return new ResponseEntity<String>(Utils.getResponseObject(e.getMessage()).toString(),
	...
	@RequestMapping(value = "/deleteProblemsByFields", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<?> deleteProblemsByFields(@RequestBody ReviewProblem param) {
		try {
			int ret = rpMapper.deleteByFields(param);
			if (ret == 0) {
				return new ResponseEntity<String>(Utils.getResponseObject(Constants.DELETE_FAILED).toString(),
						HttpStatus.BAD_REQUEST);
			} else {
				return new ResponseEntity<String>(
						Utils.getResponseObject(Constants.DELETE_SUCCESSFULLY).toString(),
						HttpStatus.OK);
			}
		} catch (Exception e) {
			return new ResponseEntity<String>(Utils.getResponseObject(e.getMessage()).toString(),
